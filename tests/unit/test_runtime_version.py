...
RELEASE_API_URL = "https://api.github.test/repos/rtx09x/agent-load-balancer/releases/latest"
RELEASE_PAGE_URL = "https://github.com/rtx09x/agent-load-balancer/releases/latest"
...
async def test_runtime_version_reports_update_available_for_newer_github_release() -> None:
    ...
    assert status.release_url == RELEASE_PAGE_URL


@pytest.mark.asyncio
async def test_runtime_version_does_not_report_update_for_same_release() -> None:
    service = RuntimeVersionService(current_version="1.19.0", ttl_seconds=60, release_api_url=RELEASE_API_URL)
...
async def test_runtime_version_reports_stable_release_as_update_for_current_beta() -> None:
    service = RuntimeVersionService(current_version="1.20.0-beta.1", ttl_seconds=60, release_api_url=RELEASE_API_URL)
    session = _mock_session(_mock_response(json_data={"tag_name": "v1.20.0"}))
...
@pytest.mark.asyncio
async def test_runtime_version_is_disabled_without_release_api_url() -> None:
    service = RuntimeVersionService(current_version="1.19.0", ttl_seconds=60)

    with patch("app.modules.runtime.service.aiohttp.ClientSession") as client_session:
        status = await service.get_version_status()
