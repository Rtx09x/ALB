...
from app.modules.agent_provider_runtime.antigravity import (
    ...
    command_preview,
    recover_antigravity_print_output,
)
...
_ANTIGRAVITY_UNSUPPORTED_FIELDS = frozenset(
    {
        "background",
...
_ANTIGRAVITY_SUPPORTED_TOOL_TYPES = frozenset({"code_execution", "google_search", "url_context"})
_ANTIGRAVITY_MODEL_ALIASES = {
    "antigravity-preview-05-2026": ("model", "gemini-3-flash-preview"),
}
_ANTIGRAVITY_SUPPORTED_AGENT_PREFIXES = ("deep-research-",)
...
class AntigravityHarnessService:
    ...
    async def print_prompt(self, request: AntigravityHarnessRequest) -> AntigravityHarnessResult:
        ...
        if process.exit_code == 0 and not process.stdout.strip():
            recovered_stdout = recover_antigravity_print_output(
                command.cwd,
        ...
        blank_success = process.exit_code == 0 and not process.stdout.strip()
        status = "success" if process.exit_code == 0 and not blank_success else "error"
        error_code = (
            None
            if process.exit_code == 0 and not blank_success
            else "antigravity_cli_empty_output"
            if blank_success
            else "antigravity_cli_failed"
        )
        error_message = (
            None
            if process.exit_code == 0 and not blank_success
            else process.stderr or "Antigravity CLI exited successfully without output"
            if blank_success
            else process.stderr or process.stdout or "Antigravity CLI failed"
        )
        await self._write_harness_log(
            request_id=request_id,
            command=command,
        ...
        if blank_success:
            raise AntigravityHarnessExecutionError(
                error_message or "Antigravity CLI exited successfully without output"
            )
        if process.exit_code == 0 and not blank_success:
...
class AntigravityManagedAgentService:
    ...
    async def create_interaction(
        ...
        policy_payload = _interaction_payload_for_api_key(payload, api_key)
        target = _interaction_target_label(policy_payload)
        request_payload = _normalize_antigravity_interaction_payload(policy_payload)
        request_id = ensure_request_id()
        started_at = time.perf_counter()
        reservation = await self._reserve_api_key_usage(context, model=target)
        ...
        await self._finalize_api_key_usage(reservation, model=target, usage=usage)
    ...
    async def complete_chat(
        ...
        api_key = None if context is None else context.api_key
        request_payload = _payload_for_api_key(payload, api_key)
        _reject_antigravity_unsupported_fields(request_payload)
        request = parse_chat_completion_request(request_payload)
        if request.stream:
...
def parse_chat_completion_request(payload: Mapping[str, JsonValue]) -> GeminiChatRequest:
    ...
    max_tokens = _int_or_none(payload.get("max_tokens"))
    if max_tokens is None:
        max_tokens = _int_or_none(payload.get("max_completion_tokens"))
    thinking_budget = _int_or_none(payload.get("thinking_budget"))
    if thinking_budget is None:
        thinking_budget = _int_or_none(payload.get("thinkingBudget"))
    thinking_level = _string_or_none(payload.get("thinking_level")) or _string_or_none(payload.get("thinkingLevel"))
    try:
...
def _interaction_payload_for_api_key(
    ...
    target = _interaction_policy_target(payload)
    if target is None:
        return payload
    enforced_model = None if api_key is None else api_key.enforced_model
    effective_target = enforced_model or target
    if api_key is not None and api_key.allowed_models and effective_target not in api_key.allowed_models:
        raise ProxyModelNotAllowed(f"Model is not allowed for this API key: {effective_target}")
    if enforced_model is None:
        return payload
    updated = dict(payload)
    if "model" in updated and "agent" not in updated:
        updated["model"] = enforced_model
    else:
...
def _normalize_antigravity_interaction_payload(payload: Mapping[str, JsonValue]) -> dict[str, JsonValue]:
    _reject_antigravity_unsupported_fields(payload)
    target_field, target_id = _interaction_target(payload)
    ...
    if store is not None and store is not True:
        raise AntigravityRuntimeValidationError("Antigravity interactions require store=true")
    tools = payload.get("tools")
    if tools is not None:
        if not is_json_list(tools):
            raise AntigravityRuntimeValidationError("tools must be an array")
        _validate_antigravity_tools(tools)
        normalized["tools"] = tools
    return normalized


def _reject_antigravity_unsupported_fields(payload: Mapping[str, JsonValue]) -> None:
    unsupported = sorted(field for field in _ANTIGRAVITY_UNSUPPORTED_FIELDS if field in payload)
    if unsupported:
        joined = ", ".join(unsupported)
        raise AntigravityRuntimeValidationError(f"Antigravity interactions do not support: {joined}")


def _validate_antigravity_tools(tools: list[JsonValue]) -> None:
    for tool in tools:
        if not is_json_mapping(tool):
            raise AntigravityRuntimeValidationError("tools entries must be objects")
        tool_type = tool.get("type")
        if not isinstance(tool_type, str) or not tool_type.strip():
            raise AntigravityRuntimeValidationError("tools entries require a type")
        if tool_type not in _ANTIGRAVITY_SUPPORTED_TOOL_TYPES:
            raise AntigravityRuntimeValidationError(f"Antigravity interactions do not support tool type: {tool_type}")


def _interaction_policy_target(payload: Mapping[str, JsonValue]) -> str | None:
...
def _interaction_target(payload: Mapping[str, JsonValue]) -> tuple[str, str]:
    agent = payload.get("agent")
    model = payload.get("model")
    if isinstance(agent, str) and agent.strip() and isinstance(model, str) and model.strip():
        raise AntigravityRuntimeValidationError("Use either agent or model, not both")
    if isinstance(agent, str) and agent.strip():
        return _interaction_agent_target(agent.strip())
    if isinstance(model, str) and model.strip():
        return _interaction_model_target(model.strip())
    raise AntigravityRuntimeValidationError("agent or model is required")


def _interaction_agent_target(agent: str) -> tuple[str, str]:
    alias = _ANTIGRAVITY_MODEL_ALIASES.get(agent)
    if alias is not None:
        return alias
    if agent.startswith(_ANTIGRAVITY_SUPPORTED_AGENT_PREFIXES):
        return ("agent", agent)
    raise AntigravityRuntimeValidationError("agent must be an Antigravity alias or supported Gemini agent")


def _interaction_model_target(model: str) -> tuple[str, str]:
    alias = _ANTIGRAVITY_MODEL_ALIASES.get(model)
    if alias is not None:
        return alias
    if model.startswith("gemini-"):
        return ("model", model)
    raise AntigravityRuntimeValidationError("model must be an Antigravity alias or Gemini model")


def _interaction_target_label(payload: Mapping[str, JsonValue]) -> str:
    value = _interaction_policy_target(payload)
    if value is None:
        raise AntigravityRuntimeValidationError("agent or model is required")
    return value
