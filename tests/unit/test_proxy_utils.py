...
async def test_open_upstream_websocket_holds_half_open_probe_until_lifecycle_finishes(monkeypatch):
    ...
    assert getattr(websocket, "_alb_half_open_probe_held", False) is True
        ...
        "tool_uses": [
            {
                ...
                "parameters": {"cmd": "gh pr view --repo rtx09x/agent-load-balancer"},
            },
            {
                "recipient_name": "functions.exec_command",
                "parameters": {"cmd": "gh pr checks --repo rtx09x/agent-load-balancer"},
            },
        ]
    }
    replay_arguments = {
        "tool_uses": [
            ...
            },
            {
                ...
                "parameters": {"repo": "rtx09x/agent-load-balancer", "path": "README.md"},
            },
        ]
    }
...
async def test_stream_responses_retries_security_work_warning_on_authorized_account(monkeypatch):
    ...
    event = json.loads(chunks[1].split("data: ", 1)[1])
    assert warning["type"] == "alb.warning"
...
async def test_stream_responses_treats_missing_security_work_pool_as_optional(monkeypatch):
    ...
    assert retry_warning["type"] == "alb.warning"
    ...
    assert missing_pool_warning["type"] == "alb.warning"
...
def test_settings_parses_image_inline_allowlist_from_csv(monkeypatch):
    monkeypatch.setenv("ALB_IMAGE_INLINE_ALLOWED_HOSTS", "a.example, b.example ,,C.Example")
    from app.core.config.settings import Settings
