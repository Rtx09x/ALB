...
Thanks for thinking about contributing to agent-load-balancer! This document covers
everything you need to get from "clone" to "merged PR" without re-discovering
the conventions yourself.

If anything below is unclear or out of date, please open a small `docs:` PR â€”
...
- **Discussions**: answer "how do Iâ€¦?" questions in
  [GitHub Discussions](https://github.com/rtx09x/agent-load-balancer/discussions).
...
[`good first issue`](https://github.com/rtx09x/agent-load-balancer/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22) or
[`help wanted`](https://github.com/rtx09x/agent-load-balancer/issues?q=is%3Aopen+is%3Aissue+label%3A%22help+wanted%22)
if you're looking for a starter task.

## Development setup

agent-load-balancer is a Python project managed with [`uv`](https://docs.astral.sh/uv/).

```bash
# 1. Fork & clone
git clone https://github.com/<your-user>/agent-load-balancer.git
cd agent-load-balancer

# 2. Install Python + deps via uv
uv sync --all-extras --dev

# 3. Activate the venv (optional â€” `uv run` works without activating)
...
uv run agent-load-balancer --help
```
...
```
.
â”œâ”€â”€ app/                  # Python application (proxy, accounts, dashboard backend)
â”œâ”€â”€ frontend/             # Dashboard SPA
â”œâ”€â”€ deploy/helm/agent-load-balancer/ # Helm chart
â”œâ”€â”€ tests/
â”‚   â”œâ”€â”€ unit/             # Fast, isolated tests
â”‚   â””â”€â”€ integration/      # End-to-end / network-touching tests
â”œâ”€â”€ openspec/             # SSOT â€” specs, change proposals, archived changes
â”‚   â”œâ”€â”€ specs/<capability>/
â”‚   â””â”€â”€ changes/<change>/
â””â”€â”€ .github/              # Issue forms, PR template, workflows
```

## Workflow: OpenSpec-first

agent-load-balancer uses **OpenSpec as the source of truth** for change-driven development.
...
and ask â€” a maintainer will tell you.
...
- **Imports / style**: follow what you see in nearby files; the linter is
  authoritative â€” if `ruff` and your style guide disagree, ruff wins.
...
agent-load-balancer uses **[Conventional Commits](https://www.conventionalcommits.org/)**
...
| `feat`      | New user-facing feature â†’ minor version bump              |
...
PR titles must follow the same format â€” that's the title release-please reads.
...
5. Codex Review (and a human maintainer) will review. Address feedback by
   pushing follow-up commits â€” no force-pushing during active review.
...
2. **`@codex review` must be clean â€” or its findings addressed â€” on the
   ...
   local run misses).
   The `ðŸ¤– codex: ok` label is maintained by the trusted
...
3. **`mergeable` must be `CLEAN`** in the GitHub API â€” no merge
...
privately via
[GitHub Security Advisories](https://github.com/rtx09x/agent-load-balancer/security/advisories/new).
See [SECURITY.md](./SECURITY.md) for full details.

---

Happy hacking. If you get stuck, open a draft PR or start a Discussion â€”
