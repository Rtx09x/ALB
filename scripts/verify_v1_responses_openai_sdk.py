...
Drives the official `openai` Python SDK against a running agent-load-balancer instance
...
Usage:
    # 1. Boot agent-load-balancer locally (with the fix branch checked out)
    cd ~/projects/agent-load-balancer
    ...
    .venv/bin/python scripts/verify_v1_responses_openai_sdk.py \\
        ...
        --api-key <agent-load-balancer dashboard key> \\
        --model gpt-5.5
...
async def case_error_stream(client: openai.AsyncOpenAI, model: str) -> CaseResult:
    ...
    try:
        ...
        try:
            ...
            # are NOT a pass â€” those mean the proxy or upstream is broken,
            ...
            # Transport-level failure â€” never a pass.
...
async def main() -> int:
    ...
    parser.add_argument("--base-url", required=True, help="agent-load-balancer /v1 base URL (e.g. http://127.0.0.1:2455/v1)")
    parser.add_argument("--api-key", required=True, help="agent-load-balancer API key (sk-clb-...)")
    ...
    for name, fn in cases:
        ...
        marker = "âœ“ PASS" if result.passed else "âœ— FAIL"
