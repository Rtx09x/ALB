...
These schemas describe the *public* surface that ALB exposes. They mirror
the OpenAI Images API request shape so SDKs (e.g. ``openai`` Python client,
the Codex CLI image fallback) can target ALB without modification.
...
def validate_image_request_parameters(
    *,
    ...
    # ALB does not yet implement client-side fan-out (multiple
