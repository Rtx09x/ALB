...
@pytest.mark.parametrize(
    ...
    [
        ...
        (None, True, False, Path("/home/user") / ".agent-load-balancer"),
        (None, True, True, DOCKER_DATA_DIR),
        (None, False, True, DOCKER_DATA_DIR),
        (None, False, False, Path("/home/user") / ".agent-load-balancer"),
    ],
)
...
def test_data_dir_from_env_file_updates_related_defaults(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ALB_DATA_DIR", raising=False)
    monkeypatch.delenv("ALB_DATABASE_URL", raising=False)
    monkeypatch.delenv("ALB_ENCRYPTION_KEY_FILE", raising=False)
    monkeypatch.delenv("ALB_CONVERSATION_ARCHIVE_DIR", raising=False)
    monkeypatch.delenv("ALB_DATA_DIR", raising=False)
    monkeypatch.delenv("ALB_DATABASE_URL", raising=False)
    monkeypatch.delenv("ALB_ENCRYPTION_KEY_FILE", raising=False)
    monkeypatch.delenv("ALB_CONVERSATION_ARCHIVE_DIR", raising=False)
    ...
    env_file.write_text(f"ALB_DATA_DIR={data_dir}\n", encoding="utf-8")
...
def test_alb_env_file_prefix_updates_related_defaults(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ALB_DATA_DIR", raising=False)
    monkeypatch.delenv("ALB_DATA_DIR", raising=False)
    monkeypatch.delenv("ALB_DATABASE_URL", raising=False)
    monkeypatch.delenv("ALB_ENCRYPTION_KEY_FILE", raising=False)
    monkeypatch.delenv("ALB_CONVERSATION_ARCHIVE_DIR", raising=False)
    data_dir = tmp_path / "alb-configured"
    monkeypatch.setenv("ALB_DATA_DIR", str(data_dir))
...
def test_alb_env_prefix_wins_over_legacy_prefix(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ALB_DATA_DIR", raising=False)
    monkeypatch.delenv("ALB_DATA_DIR", raising=False)
    monkeypatch.delenv("ALB_DATABASE_URL", raising=False)
    monkeypatch.delenv("ALB_DATABASE_URL", raising=False)
    alb_dir = tmp_path / "alb"
    legacy_dir = tmp_path / "legacy"
    monkeypatch.setenv("ALB_DATA_DIR", str(legacy_dir))
    monkeypatch.setenv("ALB_DATA_DIR", str(alb_dir))
    monkeypatch.setenv("ALB_DATABASE_URL", "sqlite+aiosqlite:///legacy.db")
    monkeypatch.setenv("ALB_DATABASE_URL", "sqlite+aiosqlite:///alb.db")

    settings = Settings()

    assert settings.data_dir == alb_dir
    assert settings.database_url == "sqlite+aiosqlite:///alb.db"
...
def test_blank_data_dir_from_env_file_uses_default_home_dir(
    ...
    monkeypatch.delenv("ALB_CONVERSATION_ARCHIVE_DIR", raising=False)
    env_file = tmp_path / ".env"
    env_file.write_text("ALB_DATA_DIR=   \n", encoding="utf-8")

    expected_data_dir = Path("/home/user") / ".agent-load-balancer"
    with (
        patch("app.core.config.settings.Path.home", return_value=Path("/home/user")),
...
def test_blank_data_dir_from_process_env_uses_default_home_dir(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ALB_DATA_DIR", "   ")
...
def test_data_dir_keeps_explicit_related_overrides(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    env_file.write_text(
        "\n".join(
            [
                f"ALB_DATA_DIR={data_dir}",
                "ALB_DATABASE_URL=sqlite+aiosqlite:///explicit.db",
                f"ALB_ENCRYPTION_KEY_FILE={key_file}",
                f"ALB_CONVERSATION_ARCHIVE_DIR={archive_dir}",
            ]
        ),
        encoding="utf-8",
    )
...
def test_data_dir_keeps_explicit_related_overrides_that_equal_old_defaults(
    ...
    env_file.write_text(
        "\n".join(
            [
                f"ALB_DATA_DIR={data_dir}",
                f"ALB_DATABASE_URL={DEFAULT_DATABASE_URL}",
                f"ALB_ENCRYPTION_KEY_FILE={DEFAULT_ENCRYPTION_KEY_FILE}",
                f"ALB_CONVERSATION_ARCHIVE_DIR={DEFAULT_CONVERSATION_ARCHIVE_DIR}",
            ]
        ),
        encoding="utf-8",
    )
