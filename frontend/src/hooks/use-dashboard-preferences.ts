...
const ACCOUNT_BURNRATE_STORAGE_KEY = "alb-account-burnrate-enabled";
const LEGACY_ACCOUNT_BURNRATE_STORAGE_KEY = "codex-lb-account-burnrate-enabled";
...
function readStoredAccountBurnrateEnabled(): boolean | null {
  ...
  const stored =
    window.localStorage.getItem(ACCOUNT_BURNRATE_STORAGE_KEY) ??
    window.localStorage.getItem(LEGACY_ACCOUNT_BURNRATE_STORAGE_KEY);
  if (stored === "true") {
    return true;
  }
  if (stored === "false") {
