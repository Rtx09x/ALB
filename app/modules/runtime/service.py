...
_GITHUB_LATEST_RELEASE_URL: str | None = None
_LATEST_RELEASE_PAGE_URL: str | None = None
...
class RuntimeVersionService:
    def __init__(
        ...
        *,
        ...
        release_api_url: str | None = _GITHUB_LATEST_RELEASE_URL,
        release_page_url: str | None = _LATEST_RELEASE_PAGE_URL,
    ) -> None:
        self._current_version = current_version
        self._ttl_seconds = ttl_seconds
        self._failure_ttl_seconds = failure_ttl_seconds
        self._github_token_env_var = github_token_env_var
        self._release_api_url = release_api_url
    ...
    async def _fetch_snapshot(self) -> _RuntimeVersionSnapshot:
        ...
        try:
            ...
            logger.warning("Failed to fetch latest Agent Load Balancer release from GitHub", exc_info=True)
            return _RuntimeVersionSnapshot(
                current_version=self._current_version,
                latest_version=None,
                update_available=False,
        ...
        return _RuntimeVersionSnapshot(
            ...
            checked_at=checked_at,
            source="github",
            release_url=self._release_page_url,
        )

    async def _fetch_latest_release_version(self) -> str:
        assert self._release_api_url is not None
        timeout = aiohttp.ClientTimeout(total=10, sock_connect=5, sock_read=5)
        headers = {
            "Accept": "application/vnd.github+json",
            "User-Agent": f"agent-load-balancer/{self._current_version}",
        }
        github_token = os.getenv(self._github_token_env_var, "").strip()
        if github_token:
            headers["Authorization"] = f"Bearer {github_token}"
        async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
            async with session.get(self._release_api_url, headers=headers) as response:
