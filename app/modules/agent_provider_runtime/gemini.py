...
def _generation_config(request: GeminiChatRequest) -> dict[str, JsonValue]:
    ...
    if thinking_config:
        config["thinkingConfig"] = cast(JsonValue, thinking_config)
    return config
...
def _thinking_config(request: GeminiChatRequest) -> dict[str, JsonValue]:
    if request.thinking_budget is not None and request.thinking_level is not None:
        raise GeminiAdapterError("Use either thinking_budget or thinking_level, not both")
