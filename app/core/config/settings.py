...
PRIMARY_ENV_PREFIX = "ALB_"
LEGACY_ENV_PREFIX = "CODEX_LB_"

DOCKER_DATA_DIR = Path("/var/lib/agent-load-balancer")
...
def _default_home_dir() -> Path:
    env_dir = _prefixed_env("DATA_DIR")
    if env_dir and env_dir.strip():
        return Path(env_dir.strip())
    home_dir = Path.home() / ".agent-load-balancer"
    if _in_container():
...
def _default_http_bridge_instance_id() -> str:
    ...
    return hostname or "agent-load-balancer"
...
def _prefixed_env(name: str) -> str | None:
    return os.getenv(f"{PRIMARY_ENV_PREFIX}{name}") or os.getenv(f"{LEGACY_ENV_PREFIX}{name}")


def _alb_settings_source(settings_cls: type["Settings"]) -> dict[str, str]:
    values: dict[str, str] = {}
    field_names = set(settings_cls.model_fields)
    for env_file in ENV_FILES:
        for key, value in dotenv_values(env_file).items():
            _add_alb_setting(values, field_names=field_names, key=key, value=value)
    for key, value in os.environ.items():
        _add_alb_setting(values, field_names=field_names, key=key, value=value)
    return values


def _add_alb_setting(
    values: dict[str, str],
    *,
    field_names: set[str],
    key: str,
    ...
    if value is None or not key.startswith(PRIMARY_ENV_PREFIX):
        return
    field_name = key.removeprefix(PRIMARY_ENV_PREFIX).lower()
...
class Settings(BaseSettings):
    ...
    def settings_customise_sources(
        ...
        return (
            init_settings,
            lambda: _alb_settings_source(cls),
            env_settings,
            dotenv_settings,
            file_secret_settings,
        )
