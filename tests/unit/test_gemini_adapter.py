...
def test_build_generate_content_payload_disables_25_flash_thinking_for_openai_max_tokens() -> None:
    payload = build_generate_content_payload(
        GeminiChatRequest(
            model="gemini-2.5-flash",
...
def test_build_generate_content_payload_maps_explicit_thinking_controls() -> None:
    budget_payload = build_generate_content_payload(
        GeminiChatRequest(
            model="gemini-2.5-flash",
            messages=[{"role": "user", "content": "Hello"}],
            thinking_budget=128,
    ...
    level_payload = build_generate_content_payload(
        GeminiChatRequest(
    ...
    assert budget_payload["generationConfig"] == {"thinkingConfig": {"thinkingBudget": 128}}
    assert level_payload["generationConfig"] == {"thinkingConfig": {"thinkingLevel": "low"}}


def test_build_generate_content_payload_rejects_conflicting_thinking_controls() -> None:
    with pytest.raises(GeminiAdapterError, match="Use either thinking_budget or thinking_level"):
        build_generate_content_payload(
            GeminiChatRequest(
                model="gemini-2.5-flash",
                messages=[{"role": "user", "content": "Hello"}],
