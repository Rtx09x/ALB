...
def test_dry_run_reports_jsonl_and_sqlite_without_writing(tmp_path: Path) -> None:
    ...
    _create_state_db(state_db, ["openai", "agent-load-balancer"])

    result = retag_codex_sessions(
        codex_home=codex_home,
        source_provider="openai",
        target_provider="agent-load-balancer",
        dry_run=True,
    )
    ...
    assert _read_state_providers(state_db) == ["openai", "agent-load-balancer"]
...
def test_retag_updates_jsonl_and_sqlite_with_backup(tmp_path: Path) -> None:
    ...
    _write_jsonl(
        ...
        [
            ...
            {"model_provider": "agent-load-balancer", "id": "b"},
        ],
    )
    _create_state_db(state_db, ["openai", "openai", "agent-load-balancer"])
    ...
    assert [record["model_provider"] for record in records] == ["agent-load-balancer", "agent-load-balancer"]
    assert _read_state_providers(state_db) == ["agent-load-balancer", "agent-load-balancer", "agent-load-balancer"]
    assert result.methods_used == ("jsonl", "sqlite")
    ...
    assert ProviderCount("agent-load-balancer", 5) in result.provider_counts_after
...
def test_retag_updates_nested_session_meta_provider(tmp_path: Path) -> None:
    ...
    records = [json.loads(line) for line in session_file.read_text(encoding="utf-8").splitlines()]
    assert records[0]["payload"]["model_provider"] == "agent-load-balancer"
    assert records[1]["payload"]["model_provider"] == "openai"
    assert result.jsonl_files_matched == 1
    assert ProviderCount("openai", 1) in result.provider_counts_before
    assert ProviderCount("agent-load-balancer", 1) in result.provider_counts_after
...
def test_retag_streams_jsonl_rewrite_to_temp_file(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    retag_codex_sessions(codex_home=codex_home, source_provider="openai", target_provider="agent-load-balancer")

    assert writes == ['{"model_provider":"agent-load-balancer","id":"a"}\n']
...
def test_retag_supports_jsonl_only_storage(tmp_path: Path) -> None:
    ...
    assert json.loads(session_file.read_text(encoding="utf-8"))["model_provider"] == "agent-load-balancer"
...
def test_retag_uses_copy_fallback_when_live_sqlite_cannot_open(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert _read_state_providers(state_db) == ["agent-load-balancer"]
...
def test_sqlite_copy_fallback_uses_consolidated_backup_and_removes_sidecars(tmp_path: Path) -> None:
    ...
    try:
        ...
        codex_sessions_retag._update_sqlite_provider_in_place(temp_path, "openai", "agent-load-balancer")
...
def test_retag_stays_inside_configured_codex_home(tmp_path: Path) -> None:
    ...
    assert _read_state_providers(mounted_db) == ["agent-load-balancer"]
