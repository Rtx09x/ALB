...
def test_cleanup_plan_closes_only_superseded_managed_beta_prs() -> None:
    ...
    prs = [
        {
            ...
            "headRepositoryOwner": {"login": "Rtx09x"},
            "labels": [],
    ...
    assert plan[0].delete_ref_api_path == "repos/rtx09x/agent-load-balancer/git/refs/heads/release/beta-1.19.1-beta.1"
...
def test_execute_cleanup_comments_closes_and_deletes_branch(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cleanup.execute_cleanup(
        [
            cleanup.CleanupAction(
                ...
                delete_ref_api_path="repos/rtx09x/agent-load-balancer/git/refs/heads/release/beta-1.19.1-beta.1",
            )
        ],
        repo="rtx09x/agent-load-balancer",
        dry_run=False,
    ...
    assert calls == [
        [
            ...
            "807",
            "--repo",
            "rtx09x/agent-load-balancer",
            "--body",
            "Superseded by v1.20.0-beta.1",
        ],
        ["pr", "close", "807", "--repo", "rtx09x/agent-load-balancer"],
        [
            "api",
            "-X",
            "DELETE",
            "repos/rtx09x/agent-load-balancer/git/refs/heads/release/beta-1.19.1-beta.1",
        ],
    ]
...
def test_cleanup_script_dry_run_lists_superseded_pr(tmp_path: Path) -> None:
    ...
    gh_cmd.write_text(f'@"{sys.executable}" "{gh}" %*\n', encoding="utf-8")
    gh.chmod(0o755)
    ...
    result = subprocess.run(
        ...
        env={
            **os.environ,
            "PATH": os.pathsep.join([str(tmp_path), str(Path("/usr/bin"))]),
