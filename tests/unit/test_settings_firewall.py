...
def test_settings_parses_firewall_trusted_proxy_cidrs_from_csv(monkeypatch):
    monkeypatch.setenv("ALB_FIREWALL_TRUSTED_PROXY_CIDRS", "127.0.0.1/32, 10.0.0.0/8")
    settings = Settings()
...
def test_settings_rejects_invalid_firewall_trusted_proxy_cidr(monkeypatch):
    monkeypatch.setenv("ALB_FIREWALL_TRUSTED_PROXY_CIDRS", "not-a-cidr")
    with pytest.raises(ValidationError):
        Settings()


def test_settings_parses_proxy_unauthenticated_client_cidrs_from_csv(monkeypatch):
    monkeypatch.setenv("ALB_PROXY_UNAUTHENTICATED_CLIENT_CIDRS", "192.168.65.1/32, 172.17.0.0/16")
...
def test_settings_parses_http_bridge_instance_ring_from_csv(monkeypatch):
    monkeypatch.setenv("ALB_HTTP_RESPONSES_SESSION_BRIDGE_INSTANCE_ID", "instance-b")
    monkeypatch.setenv("ALB_HTTP_RESPONSES_SESSION_BRIDGE_INSTANCE_RING", "instance-a, instance-b")
...
def test_settings_rejects_http_bridge_instance_id_missing_from_ring(monkeypatch):
    monkeypatch.setenv("ALB_HTTP_RESPONSES_SESSION_BRIDGE_INSTANCE_ID", "instance-c")
    monkeypatch.setenv("ALB_HTTP_RESPONSES_SESSION_BRIDGE_INSTANCE_RING", "instance-a, instance-b")
...
def test_settings_rejects_shared_http_bridge_advertise_base_url_for_multi_replica(monkeypatch):
    monkeypatch.setenv("ALB_HTTP_RESPONSES_SESSION_BRIDGE_INSTANCE_ID", "instance-a")
    monkeypatch.setenv("ALB_HTTP_RESPONSES_SESSION_BRIDGE_INSTANCE_RING", "instance-a, instance-b")
    monkeypatch.setenv(
        "ALB_HTTP_RESPONSES_SESSION_BRIDGE_ADVERTISE_BASE_URL",
        "http://agent-load-balancer-internal.default.svc.cluster.local:2455",
    )
...
def test_settings_rejects_shared_http_bridge_advertise_base_url_without_static_ring(monkeypatch):
    monkeypatch.setenv("ALB_HTTP_RESPONSES_SESSION_BRIDGE_INSTANCE_ID", "instance-a")
    monkeypatch.delenv("ALB_HTTP_RESPONSES_SESSION_BRIDGE_INSTANCE_RING", raising=False)
    monkeypatch.setenv(
        "ALB_HTTP_RESPONSES_SESSION_BRIDGE_ADVERTISE_BASE_URL",
        "http://agent-load-balancer-internal.default.svc.cluster.local:2455",
    )
...
def test_settings_allows_replica_specific_http_bridge_advertise_base_url(monkeypatch):
    ...
    monkeypatch.setenv(
        "ALB_HTTP_RESPONSES_SESSION_BRIDGE_ADVERTISE_BASE_URL",
        "http://instance-a.agent-load-balancer-bridge.default.svc.cluster.local:2455",
    )
...
def test_settings_allows_pod_specific_http_bridge_advertise_base_url_without_instance_id_label(monkeypatch):
    monkeypatch.setenv("ALB_HTTP_RESPONSES_SESSION_BRIDGE_INSTANCE_ID", "logical-instance-a")
    monkeypatch.setenv("POD_NAME", "agent-load-balancer-workload-0")
    monkeypatch.setenv(
        "ALB_HTTP_RESPONSES_SESSION_BRIDGE_ADVERTISE_BASE_URL",
        "http://agent-load-balancer-workload-0.mesh.internal:2455",
    )

    settings = Settings()

    assert settings.http_responses_session_bridge_advertise_base_url == "http://agent-load-balancer-workload-0.mesh.internal:2455"
...
def test_settings_rejects_metrics_port_matching_http_port_env(monkeypatch):
    ...
    monkeypatch.setenv("ALB_METRICS_PORT", "9090")
...
def test_dashboard_trusted_header_mode_requires_proxy_header_trust(monkeypatch):
    monkeypatch.setenv("ALB_DASHBOARD_AUTH_MODE", DashboardAuthMode.TRUSTED_HEADER)
    monkeypatch.setenv("ALB_FIREWALL_TRUST_PROXY_HEADERS", "false")
...
def test_dashboard_trusted_header_mode_accepts_valid_proxy_configuration(monkeypatch):
    monkeypatch.setenv("ALB_DASHBOARD_AUTH_MODE", DashboardAuthMode.TRUSTED_HEADER)
    monkeypatch.setenv("ALB_FIREWALL_TRUST_PROXY_HEADERS", "true")
    monkeypatch.setenv("ALB_FIREWALL_TRUSTED_PROXY_CIDRS", "127.0.0.1/32,10.0.0.0/8")
    monkeypatch.setenv("ALB_DASHBOARD_AUTH_PROXY_HEADER", "Remote-User")
...
def test_dashboard_trusted_header_mode_rejects_reserved_proxy_header(monkeypatch):
    monkeypatch.setenv("ALB_DASHBOARD_AUTH_PROXY_HEADER", "X-Forwarded-For")
