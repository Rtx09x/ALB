"""E2E tests: real OpenAI Python SDK across all supported agent-load-balancer /v1 surfaces.
...
a agent-load-balancer route, plus a parametrised audit of routes the proxy does NOT
expose â€” so that a regression to either layer (agent-load-balancer routing or
``app.modules.proxy.service``) shows up in CI without standing up a real
upstream account.

Surfaces covered:

- ``client.chat.completions.create(...)`` â€” streaming, non-streaming,
  tool-call, multi-turn
- ``client.responses.parse(text_format=PydanticModel)`` â€” structured
...
- Unsupported surfaces (embeddings, moderations, files, batches,
  fine_tuning, ``responses.retrieve/cancel/delete``) â€” assert the SDK
...
async def sdk_client(
    ...
    set up, so the SDK's HTTP traffic exercises the real agent-load-balancer routing
    layer end-to-end.
    """
...
# ---------------------------------------------------------------------------
# Unsupported routes â€” SDK must receive a clean 4xx (NotFoundError)
# ---------------------------------------------------------------------------
