...
TEST_DB_DIR = Path(tempfile.mkdtemp(prefix="agent-load-balancer-tests-"))
TEST_DB_PATH = TEST_DB_DIR / "agent-load-balancer.db"

os.environ["ALB_DATABASE_URL"] = os.environ.get(
    "ALB_TEST_DATABASE_URL", f"sqlite+aiosqlite:///{TEST_DB_PATH}"
)
os.environ["ALB_UPSTREAM_BASE_URL"] = "https://example.invalid/backend-api"
os.environ["ALB_USAGE_REFRESH_ENABLED"] = "false"
os.environ["ALB_MODEL_REGISTRY_ENABLED"] = "false"
os.environ["ALB_STICKY_SESSION_CLEANUP_ENABLED"] = "false"
os.environ["ALB_HTTP_RESPONSES_SESSION_BRIDGE_ENABLED"] = "false"
os.environ["ALB_QUOTA_PLANNER_SCHEDULER_ENABLED"] = "false"
...
def temp_key_file(monkeypatch):
    ...
    monkeypatch.setenv("ALB_ENCRYPTION_KEY_FILE", str(key_path))
