...
import type {
  AntigravityCliDiagnosticsResponse,
...
export function ProvidersPage() {
  ...
  const antigravityCliDiagnostics = useAntigravityCliDiagnostics();
  ...
  const error =
    ...
    getErrorMessageOrNull(antigravityCliDiagnostics.error) ||
  ...
  const refresh = () => {
    ...
    void antigravityCliDiagnostics.refetch();
  };
  ...
  const busy =
    ...
    antigravityCliDiagnostics.isFetching;
    ...
    <div className="animate-fade-in-up space-y-6">
      ...
      <Tabs defaultValue="gemini" className="space-y-4">
        ...
        <TabsContent value="gemini" className="space-y-4">
          ...
          />
          <AntigravityCliDiagnosticsPanel
            diagnostics={antigravityCliDiagnostics.data}
            busy={antigravityCliDiagnostics.isFetching}
            onRefresh={() => antigravityCliDiagnostics.refetch()}
          />
...
function CombinedOverview({ overview }: { overview?: AgentProviderOverview }) {
  ...
  const items = [
    ...
    { label: "Success", value: totals?.successCount ?? 0, icon: Activity },
    { label: "Errors", value: totals?.errorCount ?? 0, icon: Activity },
    { label: "Tokens", value: (totals?.inputTokens ?? 0) + (totals?.outputTokens ?? 0), icon: Gauge },
  ];
  return (
    <section className="space-y-3">
      ...
      {overview?.providers.length ? (
        <div className="overflow-x-auto rounded-lg border bg-card">
          <div className="grid min-w-[60rem] grid-cols-[1.2fr_repeat(8,minmax(4.5rem,1fr))] gap-3 border-b px-4 py-2 text-xs font-medium text-muted-foreground">
            <span>Provider</span>
          ...
          {overview.providers.map((provider) => (
            <div
              ...
              className="grid min-w-[60rem] grid-cols-[1.2fr_repeat(8,minmax(4.5rem,1fr))] gap-3 border-b px-4 py-3 text-sm last:border-b-0"
            >
              <span className="flex min-w-0 items-center gap-2 font-medium">
                <span className="truncate">{provider.displayName}</span>
                <Badge variant={provider.status === "ready" ? "default" : "secondary"}>{provider.status}</Badge>
              </span>
              ...
              <span className="text-right tabular-nums">{formatCount(provider.successCount)}</span>
              <span className="text-right tabular-nums">{formatCount(provider.errorCount)}</span>
              <span className="text-right tabular-nums">{formatCount(provider.inputTokens)}</span>
              <span className="text-right tabular-nums">{formatCount(provider.outputTokens)}</span>
            </div>
          ))}
        </div>
      ) : null}
...
function AntigravityCliDiagnosticsPanel({
  diagnostics,
  busy,
  onRefresh,
}: {
  diagnostics?: AntigravityCliDiagnosticsResponse;
  busy: boolean;
  onRefresh: () => unknown;
}) {
  const capabilities = [
    ["print", diagnostics?.printSupported],
    ["timeout", diagnostics?.printTimeoutSupported],
    ["conversation", diagnostics?.conversationSupported],
    ["add-dir", diagnostics?.addDirSupported],
    ["sandbox", diagnostics?.sandboxSupported],
    ["model", diagnostics?.modelSupported],
    ["plugins", diagnostics?.pluginSupported],
  ] as const;
    ...
    <section className="rounded-lg border bg-card p-4">
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold">Antigravity CLI diagnostics</h3>
        </div>
        <Button type="button" size="sm" variant="outline" disabled={busy} onClick={() => void onRefresh()}>
          <RefreshCw className={`h-4 w-4${busy ? " animate-spin" : ""}`} />
          Check
        </Button>
      </div>
      <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)]">
        <div className="rounded-lg border p-3">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant={diagnostics?.installed ? "default" : "secondary"}>
              {diagnostics?.installed ? "installed" : "missing"}
            </Badge>
            {diagnostics?.version ? <Badge variant="outline">{diagnostics.version}</Badge> : null}
            <Badge variant={diagnostics?.settingsExists ? "outline" : "secondary"}>
              {diagnostics?.settingsExists ? "settings found" : "settings missing"}
            </Badge>
          </div>
          <p className="mt-3 truncate text-xs text-muted-foreground">{diagnostics?.resolvedPath ?? diagnostics?.error ?? "Not checked"}</p>
          {diagnostics?.settingsPath ? (
            <p className="mt-1 truncate text-xs text-muted-foreground">{diagnostics.settingsPath}</p>
          ) : null}
        </div>
        <div className="rounded-lg border p-3">
...
function AntigravityHarnessPanel({
  ...
  return (
    <section className="rounded-lg border bg-card p-4">
      ...
      >
        <div className="grid gap-3 lg:grid-cols-[minmax(16rem,1fr)_minmax(12rem,0.8fr)_7rem_auto]">
          <Input
            value={workspacePath}
