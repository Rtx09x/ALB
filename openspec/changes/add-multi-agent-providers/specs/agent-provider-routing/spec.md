...
- **AND** the prompt is passed as the `--print` flag value rather than through an unsupported prompt flag
...
- **AND** an exit-zero CLI run with blank stdout is recovered from the current workspace's Antigravity conversation
  database when a current assistant answer is present
- **AND** an exit-zero CLI run with blank stdout and no recoverable current assistant answer is treated as a failed
  harness execution and does not settle usage
