...
async def test_v1_models_list(async_client):
    ...
    gemini = next(item for item in data if item["id"] == "gemini-3-pro-preview")
    assert gemini["owned_by"] == "agent-load-balancer"
    ...
    assert gemini["capabilities"]["supportsImages"] is True
    assert gemini["capabilities"]["supports_images"] is True
    assert gemini["capabilities"]["supports_vision"] is True
    assert gemini["supportsImages"] is True
    assert gemini["supports_images"] is True
    assert gemini["supportsVision"] is True
    ...
    for item in data:
        ...
        if item.get("provider") in {"gemini", "antigravity"}:
            assert item["owned_by"] == "agent-load-balancer"
            continue
        assert item["owned_by"] == "agent-load-balancer"
...
async def test_v1_models_filters_to_allowed_gemini_model(async_client):
    ...
    created = await async_client.post(
        ...
        json={
            ...
            "allowedModels": ["gemini-3-pro-preview"],
            "enforcedModel": "gemini-3-pro-preview",
        },
    )
    assert created.status_code == 200
    key = created.json()["key"]
    ...
    assert resp.status_code == 200
    data = resp.json()["data"]
    assert [entry["id"] for entry in data] == ["gemini-3-pro-preview"]
...
async def test_model_sets_are_consistent_across_api_endpoints(async_client):
    ...
    assert "gemini-3-pro-preview" in dashboard_ids
    gemini = next(item for item in dashboard.json()["models"] if item["id"] == "gemini-3-pro-preview")
    assert gemini["provider"] == "gemini"
