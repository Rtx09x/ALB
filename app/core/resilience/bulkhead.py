...
class BulkheadMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        if is_memory_pressure():
            message = "agent-load-balancer is temporarily unavailable due to local memory pressure"
        ...
        if sem.locked():
            message = f"agent-load-balancer is temporarily overloaded in the {lane} lane"
            await self._log_rejection(
                path=path,
                scope_type=scope["type"],
                lane=lane,
