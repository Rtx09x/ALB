...
The harness passes the user prompt as the value to `--print` because current
`agy` exposes `--prompt` as an alias for print mode, not as the prompt payload.
When `agy` exits 0 with blank stdout, the harness attempts a read-only recovery
from the current workspace's latest Antigravity CLI conversation database under
`~/.gemini/antigravity-cli/conversations/`. Local `agy` can generate a valid
assistant answer while failing to print it to stdout. If ALB recovers a current
assistant text payload, the probe is successful and usage settles once. If no
current answer can be recovered, the blank stdout remains a failed harness run
and provider usage is not settled.
