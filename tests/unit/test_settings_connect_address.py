...
Behaviour exercised (mirrors `app/modules/settings/api.py:48-63`):
  ...
  1. The `ALB_CONNECT_ADDRESS` env override wins over everything.
  ...
  5. Loopback hosts (`localhost`, `127.0.0.1`, `::1`) and missing host
     fall back to the `<alb-ip-or-dns>` placeholder.
"""
...
def test_env_override_wins_over_request_host(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ALB_CONNECT_ADDRESS", raising=False)
    monkeypatch.setenv("ALB_CONNECT_ADDRESS", "lb.internal.example:2455")
    request = _fake_request("10.0.0.5")
    assert _resolve_runtime_connect_address(request) == "lb.internal.example:2455"


def test_alb_env_override_wins_over_legacy_override(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("ALB_CONNECT_ADDRESS", "alb.internal.example:2455")
    monkeypatch.setenv("ALB_CONNECT_ADDRESS", "legacy.internal.example:2455")
    request = _fake_request("10.0.0.5")
    assert _resolve_runtime_connect_address(request) == "alb.internal.example:2455"


def test_env_override_is_stripped(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ALB_CONNECT_ADDRESS", raising=False)
    monkeypatch.setenv("ALB_CONNECT_ADDRESS", "  lb.internal:2455  ")
...
def test_env_override_ignored_when_empty_string(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("ALB_CONNECT_ADDRESS", raising=False)
    monkeypatch.setenv("ALB_CONNECT_ADDRESS", "   ")
    request = _fake_request("10.0.0.5")
...
def test_non_loopback_ipv4_host_returned_verbatim(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # No need to mock getaddrinfo â€” the helper must short-circuit before
...
def test_loopback_host_returns_placeholder(monkeypatch: pytest.MonkeyPatch, hostname: str) -> None:
    ...
    assert _resolve_runtime_connect_address(request) == "<alb-ip-or-dns>"
