...
def _http_bridge_startup_wait_timeout_error(
    ...
    *,
    ...
    message = f"agent-load-balancer is temporarily overloaded during {stage}"
...
def _normalized_http_bridge_instance_ring(settings: Settings) -> tuple[str, tuple[str, ...]]:
    ...
    if not instance_id:
        instance_id = "agent-load-balancer"
...
async def _active_http_bridge_instance_ring(
    ...
    try:
        ...
        logger.warning("Bridge ring lookup failed â€” refusing to fall back to static ring", exc_info=True)
        raise
