...
async def internal_bridge_responses(
    ...
    return await _stream_responses(
        ...
        # origin instance â€” the one that actually responds to the client â€” so
...
async def v1_images_variations(
    ...
    return _logged_error_json_response(
        ...
        content=images_service_module.make_not_found_error(
            "/v1/images/variations is not supported by agent-load-balancer. "
            "Use /v1/images/edits with an explicit prompt instead."
        ),
    )
    ...
    *,
    ...
    Args:
        ...
        enforce_openai_sdk_contract: when True (the default, used for /v1),
            ...
            forwarded verbatim and no synthesis happens â€” the Codex CLI
...
def _synthetic_response_created_envelope(
    ...
    ``response.created`` to be the first event the stream emits â€” the OpenAI
