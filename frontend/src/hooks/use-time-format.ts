...
const TIME_FORMAT_STORAGE_KEY = "alb-time-format";
const LEGACY_TIME_FORMAT_STORAGE_KEY = "codex-lb-time-format";
...
function readStoredPreference(): TimeFormatPreference {
  ...
  try {
    const stored =
      ...
      window.localStorage.getItem(LEGACY_TIME_FORMAT_STORAGE_KEY);
