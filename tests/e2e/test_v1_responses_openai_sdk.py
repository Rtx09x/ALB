"""E2E tests: real OpenAI Python SDK against agent-load-balancer /v1/responses via ASGI.
...
cover the entire ``agent-load-balancer`` request/response path â€” including
``_normalize_public_responses_stream`` â€” without requiring a real Codex
account.
...
def _codex_rate_limits_event() -> str:
    ...
    (intermittently â€” throttled per rate-limit window). This event causes the
...
def _response_completed_empty(resp_id: str, seq: int) -> str:
    ...
    intermediate output_item.done events (which agent-load-balancer must backfill)."""
    return _sse(
        {
...
def _response_failed_only(resp_id: str) -> str:
    """Upstream rejection mid-stream â€” response.failed without response.created.
...
async def test_sdk_responses_stream_upstream_rejection_synthesizes_created(
    ...
    async with sdk_client.responses.stream(
        ...
        # which is the SDK's normal contract â€” the test asserts the stream
