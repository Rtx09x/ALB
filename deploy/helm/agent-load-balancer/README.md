# agent-load-balancer Helm Chart

Production-ready Helm chart for [agent-load-balancer](https://github.com/rtx09x/agent-load-balancer), an OpenAI API load balancer with account pooling, usage tracking, and dashboard.
...
```bash
helm install agent-load-balancer oci://ghcr.io/rtx09x/charts/agent-load-balancer \
...
```bash
helm dependency build deploy/helm/agent-load-balancer/
helm upgrade --install agent-load-balancer deploy/helm/agent-load-balancer/ \
  -f deploy/helm/agent-load-balancer/values-bundled.yaml \
  --set postgresql.auth.password=change-me
```
...
helm install agent-load-balancer oci://ghcr.io/rtx09x/charts/agent-load-balancer \
  ...
  --set externalDatabase.url='postgresql+asyncpg://user:pass@db.example.com:5432/alb'
```
...
helm install agent-load-balancer oci://ghcr.io/rtx09x/charts/agent-load-balancer \
  --set postgresql.enabled=false \
  --set externalDatabase.existingSecret=agent-load-balancer-db \
  --set auth.existingSecret=agent-load-balancer-app
```
...
```bash
helm upgrade --install agent-load-balancer deploy/helm/agent-load-balancer/ \
  -f deploy/helm/agent-load-balancer/values-external-db.yaml \
  --set externalDatabase.url='postgresql+asyncpg://user:pass@db.example.com:5432/alb'
```
...
```bash
helm upgrade --install agent-load-balancer deploy/helm/agent-load-balancer/ \
  -f deploy/helm/agent-load-balancer/values-external-secrets.yaml \
  --set externalSecrets.secretStoreRef.name=my-store
```
...
No repo clone required â€” install directly from the OCI registry.
...
When `externalDatabase.existingSecret` is set and `auth.existingSecret` is not, the chart-managed app secret contains only the encryption key; the StatefulSet reads `ALB_DATABASE_URL` from the external DB secret.
...
```
total_connections = (databasePoolSize + databaseMaxOverflow) Ã— replicas
```
...
- StatefulSet name: `<release>-agent-load-balancer-workload`
- `serviceName: <release>-agent-load-balancer-bridge` on the StatefulSet
- `ALB_HTTP_RESPONSES_SESSION_BRIDGE_INSTANCE_ID=$(POD_NAME)`
- `ALB_HTTP_RESPONSES_SESSION_BRIDGE_ADVERTISE_BASE_URL=http://$(POD_NAME).<headless-service>.$(POD_NAMESPACE).svc.<clusterDomain>:2455`
...
```yaml
config:
  sessionBridgeAdvertiseBaseUrl: "http://agent-load-balancer-internal.default.svc.cluster.local:2455"
```

When `networkPolicy.enabled=true`, the chart also allows port `2455` traffic between agent-load-balancer pods so owner handoff can work without extra rules.
...
```yaml
config:
  sessionBridgeInstanceRing: "agent-load-balancer-0.agent-load-balancer.default.svc.cluster.local,agent-load-balancer-1.agent-load-balancer.default.svc.cluster.local"
```
...
```
(databasePoolSize + databaseMaxOverflow) Ã— maxReplicas â‰¤ PostgreSQL max_connections
```
...
Calculation: `(3 + 2) Ã— 20 = 100` connections, which fits within PostgreSQL's default `max_connections=100`.
...
```bash
helm install agent-load-balancer oci://ghcr.io/rtx09x/charts/agent-load-balancer \
  -f deploy/helm/agent-load-balancer/values-prod.yaml \
  --set externalDatabase.url='postgresql+asyncpg://user:pass@db.example.com:5432/alb'
```
...
```
preStopSleepSeconds (15s minimum) â†’ shutdownDrainTimeoutSeconds (30s) â†’ terminationGracePeriodSeconds (60s)
```
...
3. **terminationGracePeriodSeconds (60s)**: Hard deadline
   ...
   - Must be â‰¥ `preStopSleepSeconds + shutdownDrainTimeoutSeconds`
...
ingress:
  ...
  hosts:
    - host: agent-load-balancer.example.com
      paths:
        - path: /
          pathType: Prefix
```
...
gatewayApi:
  ...
  hostnames:
    - agent-load-balancer.example.com
```

## Upgrade Contract

```bash
helm upgrade agent-load-balancer oci://ghcr.io/rtx09x/charts/agent-load-balancer <your values...>
```
...
```bash
helm test agent-load-balancer -n <namespace>
...
```bash
kubectl port-forward svc/agent-load-balancer 2455:2455 -n <namespace>
...
```bash
kubectl describe pod -l app.kubernetes.io/name=agent-load-balancer -n <namespace>
