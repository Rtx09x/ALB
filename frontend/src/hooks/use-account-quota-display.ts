...
const QUOTA_DISPLAY_STORAGE_KEY = "alb-account-quota-display";
const LEGACY_QUOTA_DISPLAY_STORAGE_KEY = "codex-lb-account-quota-display";
...
function readStoredPreference(): AccountQuotaDisplayPreference {
  ...
  try {
    const stored =
      window.localStorage.getItem(QUOTA_DISPLAY_STORAGE_KEY) ??
      window.localStorage.getItem(LEGACY_QUOTA_DISPLAY_STORAGE_KEY);
