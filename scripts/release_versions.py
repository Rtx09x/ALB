"""Release version helpers for agent-load-balancer release workflows.
...
def update_project_versions(root: Path, version: str) -> None:
    ...
    chart = root / "deploy" / "helm" / "agent-load-balancer" / "Chart.yaml"
    ...
    if count != 1:
        raise ValueError("expected exactly one agent-load-balancer package entry in uv.lock")
...
def read_project_versions(root: Path) -> dict[str, str]:
    ...
    chart_text = (root / "deploy" / "helm" / "agent-load-balancer" / "Chart.yaml").read_text(encoding="utf-8")
    ...
    return {
        ...
        "deploy/helm/agent-load-balancer/Chart.yaml version": find(r"^version: (.+)$", chart_text, "chart version"),
        "deploy/helm/agent-load-balancer/Chart.yaml appVersion": find(
            r"^appVersion: (.+)$",
            chart_text,
            "chart appVersion",
        ),
        "uv.lock": find(
            r'\[\[package\]\]\nname = "agent-load-balancer"\nversion = "([^"]+)"\nsource = \{ editable = "\." \}',
            uv_text,
            "uv.lock agent-load-balancer version",
        ),
    }
