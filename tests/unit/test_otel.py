...
def test_init_tracing_returns_true_when_opentelemetry_modules_are_available(monkeypatch: pytest.MonkeyPatch):
    ...
    assert otel.init_tracing(service_name="agent-load-balancer", endpoint="http://collector:4317") is True
    ...
    assert state.resource_attributes == {"service.name": "agent-load-balancer"}
