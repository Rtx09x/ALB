...
# GPT-5 family models, but the ChatGPT backend ALB proxies to does not
...
# Service tier values ALB accepts at the API-key surface but that the
...
def normalize_unsupported_reasoning_effort(
    ...
    *,
    ...
    Some efforts that ALB accepts at the API surface (notably
...
def enforce_strict_function_tools_format(
    ...
    *,
    ...
    same payload, so ALB pre-validates here.
