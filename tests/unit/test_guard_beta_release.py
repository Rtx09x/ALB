...
def write_minimal_release_files(root: Path, version: str = "1.20.0") -> None:
    ...
    (root / "deploy" / "helm" / "agent-load-balancer").mkdir(parents=True)
    ...
    (root / "deploy" / "helm" / "agent-load-balancer" / "Chart.yaml").write_text(
        f"apiVersion: v2\nname: agent-load-balancer\nversion: {version}\nappVersion: {version}\n",
        encoding="utf-8",
    )
...
def event_file(
    ...
    *,
    ...
    head_repo: str = "rtx09x/agent-load-balancer",
    base_repo: str = "rtx09x/agent-load-balancer",
) -> Path:
...
def test_pr_guard_rejects_forked_canonical_beta_branch(tmp_path: Path) -> None:
    ...
    event = event_file(
        ...
        head_repo="evil-fork/agent-load-balancer",
    )
    ...
    assert "Expected head repository: rtx09x/agent-load-balancer" in result.stderr
    assert "Actual head repository: evil-fork/agent-load-balancer" in result.stderr
