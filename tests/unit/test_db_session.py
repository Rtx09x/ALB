...
def test_import_session_with_sqlite_memory_url_does_not_error() -> None:
    ...
    env = os.environ.copy()
    env["ALB_DATABASE_URL"] = "sqlite+aiosqlite:///:memory:"
...
def test_import_session_with_postgres_url_does_not_error() -> None:
    repo_root = Path(__file__).resolve().parents[2]
    env = os.environ.copy()
    env["ALB_DATABASE_URL"] = "postgresql+asyncpg://ALB:ALB@127.0.0.1:5432/ALB"
...
def test_postgres_engine_kwargs_enable_pre_ping_and_recycle(monkeypatch) -> None:
    ...
    """
    monkeypatch.setenv("ALB_TEST_DATABASE_URL", "")
    monkeypatch.delenv("ALB_TEST_DATABASE_URL", raising=False)
    monkeypatch.setattr(
        session_module,
        "_settings",
        _FakeSettings(
...
def test_postgres_engine_kwargs_use_nullpool_under_test_db_url(monkeypatch) -> None:
    """The ALB_TEST_DATABASE_URL escape hatch keeps NullPool semantics â€”
    pool_pre_ping/recycle are irrelevant when each session opens a fresh
    connection.
    """
    monkeypatch.setenv("ALB_TEST_DATABASE_URL", "1")
    monkeypatch.setattr(
        session_module,
        "_settings",
...
async def test_init_background_db_uses_main_pool_size_for_postgres_by_default() -> None:
    ...
    if os.environ.get("ALB_TEST_DATABASE_URL"):
