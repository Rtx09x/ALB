...
# 6 Ã— 10s (default interval) = 60s.  Combined with the 0.5s startup-probe
# window this ensures the client sees a terminal event within â‰ˆ70s when the
...
_SECURITY_WORK_RETRY_MESSAGE = (
    ...
    "agent-load-balancer is retrying on an account marked as authorized for security work."
)
_SECURITY_WORK_NO_AUTHORIZED_ACCOUNTS_MESSAGE = (
    ...
    "security work. agent-load-balancer is continuing with normal account selection; the upstream request may still "
    "fail until an account with Trusted Access for Cyber is marked as security-work-authorized."
)
...
class ProxyService(
    ...
    def __init__(self, repo_factory: ProxyRepoFactory) -> None:
        ...
        # In-memory pin from upstream-issued file_id -> ALB account_id.
    ...
    async def _accounts_refresh_scope(self) -> AsyncIterator[AccountsRepositoryPort]:
        ...
        # a background-pool connection (the original pool-exhaustion leak).
...
_FAILED_ACCOUNT_ATTR = "_alb_failed_account"
