...
@pytest.mark.asyncio
async def test_v1_chat_completions_codex_model_does_not_call_agent_provider_runtimes(async_client, monkeypatch):
    email = "chat-codex-provider-split@example.com"
    raw_account_id = "acc_chat_codex_provider_split"
    auth_json = _make_auth_json(raw_account_id, email)
    files = {"auth_json": ("auth.json", json.dumps(auth_json), "application/json")}
    response = await async_client.post("/api/accounts/import", files=files)
    assert response.status_code == 200

    async def fail_gemini_complete(*_args, **_kwargs):
        raise AssertionError("Codex chat completion called Gemini runtime")

    async def fail_gemini_stream(*_args, **_kwargs):
        raise AssertionError("Codex chat completion called Gemini stream runtime")
        yield ""

    async def fail_antigravity_complete(*_args, **_kwargs):
        raise AssertionError("Codex chat completion called Antigravity runtime")

    async def fake_stream(payload, headers, access_token, account_id, base_url=None, raise_for_status=False):
        assert payload.model == "gpt-5.2"
        yield 'data: {"type":"response.output_text.delta","delta":"codex"}\n\n'
        yield 'data: {"type":"response.completed","response":{"id":"resp_codex"}}\n\n'

    monkeypatch.setattr(runtime_service_module.GeminiRuntimeService, "complete_chat", fail_gemini_complete)
    monkeypatch.setattr(runtime_service_module.GeminiRuntimeService, "stream_chat", fail_gemini_stream)
    monkeypatch.setattr(
        runtime_service_module.AntigravityManagedAgentService,
        "complete_chat",
        fail_antigravity_complete,
    )
    monkeypatch.setattr(proxy_module, "core_stream_responses", fake_stream)
    ...
    body = resp.json()
    assert body["object"] == "chat.completion"
    assert body["choices"][0]["message"]["content"] == "codex"
