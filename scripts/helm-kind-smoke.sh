...
CHART_DIR="${ROOT_DIR}/deploy/helm/agent-load-balancer"
KUBE_CONTEXT="${KUBE_CONTEXT:-kind-agent-load-balancer-smoke}"
IMAGE_REGISTRY="${IMAGE_REGISTRY:-ghcr.io}"
IMAGE_REPOSITORY="${IMAGE_REPOSITORY:-rtx09x/agent-load-balancer}"
...
install_bundled() {
  local namespace="agent-load-balancer-smoke-bundled"
  local release="agent-load-balancer-bundled"
  ...
  helm upgrade --install "${release}" "${CHART_DIR}" \
    ...
    --set-string 'ingress.hosts[0].host=agent-load-balancer.localtest.me' \
...
install_external_db() {
  local namespace="agent-load-balancer-smoke-external"
  local release="agent-load-balancer-external"
  local db_release="agent-load-balancer-smoke-db"
  local app_secret="agent-load-balancer-external-secrets"
...
PY
  ...
  helm upgrade --install "${db_release}" oci://registry-1.docker.io/bitnamicharts/postgresql \
    ...
    --namespace "${namespace}" \
    --create-namespace \
    --set auth.username=alb \
    --set auth.password="${DB_PASSWORD}" \
    --set auth.database=alb \
  ...
  kubectl --context "${KUBE_CONTEXT}" -n "${namespace}" create secret generic "${app_secret}" \
    --from-literal=database-url="postgresql+asyncpg://alb:${DB_PASSWORD}@${db_release}-postgresql:5432/alb" \
