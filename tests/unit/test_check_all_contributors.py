...
def test_pull_request_commit_authors_include_normal_email_contributors(tmp_path, monkeypatch):
    ...
    event_path.write_text(
        json.dumps(
            {
                "pull_request": {
                    "commits_url": "https://api.github.test/repos/example/agent-load-balancer/pulls/1/commits",
    ...
    assert requested_urls == ["https://api.github.test/repos/example/agent-load-balancer/pulls/1/commits?per_page=100"]
