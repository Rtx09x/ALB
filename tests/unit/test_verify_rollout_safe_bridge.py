...
async def test_trigger_restart_uses_configured_kubectl_binary(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    config = verify_script.VerifyConfig(
        ...
        host="agent-load-balancer.localtest.me",
        api_key="sk-test",
        model="gpt-5.1",
        sessions=1,
        turns=1,
        ...
        rollout_namespace="agent-load-balancer-e2e",
        rollout_context="kind-agent-load-balancer-local",
        rollout_target="statefulset/agent-load-balancer-e2e-workload",
        delete_owner_after_first_turn=False,
        postgresql_pod="postgresql-0",
        postgresql_user="alb",
        postgresql_database="alb",
    ...
    assert seen["cmd"] == [
        "/tmp/custom-kubectl",
        "--context",
        "kind-agent-load-balancer-local",
        "-n",
        "agent-load-balancer-e2e",
        "rollout",
        "restart",
        "statefulset/agent-load-balancer-e2e-workload",
    ]
