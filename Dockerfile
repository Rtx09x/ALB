...
RUN adduser --disabled-password --gecos "" app \
    && mkdir -p /var/lib/agent-load-balancer \
    && chown -R app:app /var/lib/agent-load-balancer
...
RUN chmod +x /app/scripts/docker-entrypoint.sh \
    && printf '%s\n' '#!/bin/sh' 'exec python -m app.cli "$@"' > /usr/local/bin/agent-load-balancer \
    && printf '%s\n' '#!/bin/sh' 'exec python -m app.cli "$@"' > /usr/local/bin/alb \
    && printf '%s\n' '#!/bin/sh' 'exec python -m app.cli "$@"' > /usr/local/bin/codex-lb \
    && chmod +x /usr/local/bin/agent-load-balancer /usr/local/bin/alb /usr/local/bin/codex-lb
