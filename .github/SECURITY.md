...
[Private Vulnerability Reporting](https://github.com/rtx09x/agent-load-balancer/security/advisories/new):

1. Open <https://github.com/rtx09x/agent-load-balancer/security/advisories/new>
...
`[agent-load-balancer security]`.

## What to include

A good report makes triage much faster:

- **Affected version(s)**: e.g. `agent-load-balancer 1.16.0`, ghcr image digest, or commit SHA.
...
- **Reproduction**: minimal steps â€” config snippet, request payload, log
...
- The agent-load-balancer proxy (`app/`) â€” auth, routing, account management, the
  dashboard backend, the `/v1/*` and `/backend-api/*` surfaces.
- The dashboard frontend (`frontend/`).
- The published Docker image (`ghcr.io/rtx09x/agent-load-balancer`) and Helm chart
  (`oci://ghcr.io/rtx09x/charts/agent-load-balancer`).
- Released PyPI artifacts (`agent-load-balancer` on PyPI).
...
- Vulnerabilities in third-party dependencies that don't reach agent-load-balancer's
  attack surface â€” file those upstream.
...
1. **Acknowledge** â€” maintainer confirms receipt within **3 business days**.
2. **Triage & validate** â€” within **7 business days** the maintainer either
   confirms the issue, asks for more information, or declines with reasoning.
3. **Fix & coordinate** â€” for confirmed issues, the maintainer drafts a fix
   privately. We aim to ship a patched release within **30 days** of
   confirmation; severe issues are prioritized.
4. **Release & advisory** â€” once the patched release is published, the
   maintainer publishes a GitHub Security Advisory (with a CVE if applicable)
   crediting you, unless you request anonymity.
5. **Embargo** â€” please do not publicly disclose the issue until the advisory
...
backports â€” please upgrade.

| Version    | Supported          |
|------------|--------------------|
| 1.17.x     | âœ… active           |
| 1.16.x     | âœ… critical fixes   |
| < 1.16     | âŒ upgrade required |
