...
def _set_dashboard_auth_env(
    ...
    *,
    ...
    monkeypatch.setenv("ALB_DASHBOARD_AUTH_MODE", mode)
    monkeypatch.setenv("ALB_FIREWALL_TRUST_PROXY_HEADERS", str(trust_proxy_headers).lower())
    monkeypatch.setenv("ALB_FIREWALL_TRUSTED_PROXY_CIDRS", trusted_proxy_cidrs)
    monkeypatch.setenv("ALB_DASHBOARD_AUTH_PROXY_HEADER", proxy_header)
...
def _set_proxy_unauthenticated_client_cidrs_env(
    ...
    *,
    ...
    monkeypatch.setenv("ALB_PROXY_UNAUTHENTICATED_CLIENT_CIDRS", cidrs)
    get_settings.cache_clear()
...
async def test_remote_first_run_requires_bootstrap_token(app_instance, monkeypatch):
    monkeypatch.setenv("ALB_DASHBOARD_BOOTSTRAP_TOKEN", "bootstrap-secret")
    from app.core.config.settings import get_settings
    from app.core.config.settings_cache import get_settings_cache
