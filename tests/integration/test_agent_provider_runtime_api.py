...
import app.modules.agent_provider_runtime.api as runtime_api_module
import app.modules.agent_provider_runtime.service as runtime_service_module
from app.modules.agent_provider_runtime.antigravity import AntigravityCliDiagnosticsResult, AntigravityProcessResult
...
async def test_antigravity_harness_print_route_uses_selected_cli_profile(async_client, monkeypatch, tmp_path) -> None:
    ...
    assert payload["command"][:4] == ["agy", "--print", "<redacted>", "--print-timeout"]
...
@pytest.mark.asyncio
async def test_antigravity_cli_diagnostics_route_reports_no_quota_capabilities(async_client, monkeypatch) -> None:
    async def _inspect():
        return AntigravityCliDiagnosticsResult(
            executable="agy",
            resolved_path="C:\\tools\\agy.exe",
            installed=True,
            version="1.0.2",
            settings_path="C:\\Users\\test\\.gemini\\antigravity-cli\\settings.json",
            settings_exists=True,
            print_supported=True,
            print_timeout_supported=True,
            conversation_supported=True,
            add_dir_supported=True,
            sandbox_supported=True,
            model_supported=True,
            plugin_supported=True,
        )

    monkeypatch.setattr(runtime_api_module, "inspect_antigravity_cli", _inspect)

    response = await async_client.get("/api/agent-providers/antigravity/cli/diagnostics")
    ...
    assert payload["printSupported"] is True
    assert payload["modelSupported"] is True
    assert payload["settingsExists"] is True
...
async def test_antigravity_interactions_route_uses_api_key_account(async_client, monkeypatch) -> None:
    ...
    assert call["json"]["model"] == "gemini-3-flash-preview"
    preflight = await async_client.post("/api/agent-providers/antigravity/routing/preflight")
    assert preflight.status_code == 200
    windows = {
        window["dimension"]: window["used"]
...
async def test_antigravity_dashboard_interaction_run_uses_api_key_account(async_client, monkeypatch) -> None:
    ...
    assert session.calls[0]["json"]["store"] is True
...
@pytest.mark.asyncio
async def test_antigravity_interactions_route_rejects_unsupported_tool_types(async_client) -> None:
    key = await _create_proxy_key(async_client, {"allowedModels": []})
    await _create_antigravity_api_key_account(async_client)
    ...
    assert response.json()["error"]["message"] == (
        "Antigravity interactions do not support tool type: function_calling"
    )


@pytest.mark.asyncio
async def test_antigravity_chat_compat_rejects_unsupported_generation_fields(async_client) -> None:
    key = await _create_proxy_key(async_client, {"allowedModels": []})
    await _create_antigravity_api_key_account(async_client)
    ...
    assert response.status_code == 400
    assert response.json()["error"]["message"] == "Antigravity interactions do not support: temperature"
