...
def run_upgrade(
    ...
    *,
    ...
    config.attributes["alb_fresh_install"] = (
    ...
    if bootstrap_legacy:
        ...
        if bootstrap_result.stamped_revision is not None:
            config.attributes["alb_fresh_install"] = False
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Database migration utility for Agent Load Balancer.")
    parser.add_argument(
        "--db-url",
        default=None,
        help="Database URL to migrate. Defaults to ALB_DATABASE_URL from settings.",
    )
